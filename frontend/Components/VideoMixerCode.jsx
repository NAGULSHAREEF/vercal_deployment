...
const VideoMixerCode = () => {
  ...
  const handleMixVideos = async () => {
    if (video1 && video2) {
      ...
      try {
        const response = await axios.post('https://vercel-deployment-backend.vercel.app/mix-videos', formData, {
          headers: {
        ...
        <div className="mixed-video-section">
          ...
          <div className="mt-3">
            <a href={mixedVideoUrl} download="mixed_video.mp4" className="download-btn" style={{ backgroundColor: '#28a745', color: '#fff', padding: '10px 20px', borderRadius: '10px' }}>
              <img src="/download-png.png" alt="Download Icon" style={{ width: '20px', height: '20px', borderRadius: '50%' }} />
